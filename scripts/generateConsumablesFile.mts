...
const unpricedConsumables = result.allConsumables?.nodes?.filter(r => r !== null).map(r => ({
  id: r.id,
  liver: r.liver,
...
const date = new Date();
await fs.writeFile(`./consumables-${date.getDate().toString().padStart(2, "0")}${(date.getMonth()+1).toString().padStart(2, "0")}${date.getFullYear()}.json`, JSON.stringify(consumables));
